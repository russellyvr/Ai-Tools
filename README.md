...
| [`council-skill/`](council-skill/) | A consensus-gated AI Council: three fixed vendor seats (Anthropic, Google, OpenAI), each filled at run time with that vendor's most capable deep-reasoning model, answer independently, peer-review each other anonymously, and iterate through fresh-context checks to a strict all-seats APPROVE that preserves dissent and states its own flip condition. Revised 2026-09-12 after a council review of its own specification, then measured the same day in a nine-brief paired-evaluation pilot (no accuracy gain over a single grounded model was found; the review stage caught four confident seat errors) — see its `EVALUATION.md`. | [README](council-skill/README.md) · [docs](council-skill/docs/index.html) · [changelog](council-skill/CHANGELOG.md) |
...
deployment. `llm-cost-optimization-playbook` is a separate, self-contained
document distilled from Anthropic's published cost guidance — it installs
nothing and is independent of the two routing packages. `council-skill` and
`evidence-ingest-skill` are each independent of the rest.
